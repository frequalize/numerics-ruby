...
class ConnectionTest < Test::Unit::TestCase
  ...
  def test_003_read
    ...
    assert_equal [[t.strftime("%Y-%m-%d %H:%M:%S %z"), 4, {}], [(t+2).strftime("%Y-%m-%d %H:%M:%S %z"), 3, {}]], conn.entries('coffees')[:data]
    assert_equal [[t.strftime("%Y-%m-%d %H:%M:%S %z"), (t+2).strftime("%Y-%m-%d %H:%M:%S %z")],[4, 3]], conn.series('coffees')[:data]

    assert_equal [[(t+2).strftime("%Y-%m-%d %H:%M:%S %z"), 3, {}]], conn.entries('coffees', :limit => 1)[:data]
    assert_equal [[t.strftime("%Y-%m-%d %H:%M:%S %z"), 4, {}]], conn.entries('coffees', :start => 0, :limit => 1)[:data]
    assert_equal [[(t+2).strftime("%Y-%m-%d %H:%M:%S %z"), 3, {}]], conn.entries('coffees', :start => -1)[:data]
    assert_equal [[t.strftime("%Y-%m-%d %H:%M:%S %z"), 4, {}], [(t+2).strftime("%Y-%m-%d %H:%M:%S %z"), 3, {}]], conn.entries('coffees', :from => td)[:data]
    assert_equal [], conn.entries('coffees', :from => td + 84600)[:data]

    assert_equal [[td.strftime("%Y-%m-%d %H:%M:%S %z"), 7, {}]], conn.entries(['coffees', 'total/d'])[:data]
    assert_equal [[td.strftime("%Y-%m-%d %H:%M:%S %z"), 3.5, {}]], conn.entries(['coffees', 'ave/d'])[:data]
  ...
  def test_004_derive
    t = Time.parse("2011-10-26 11:15:44 +0100")
    tw = Time.parse("2011-10-24 00:00:00 +0000")
    assert_equal [[t.strftime("%Y-%m-%d %H:%M:%S %z"), 4, {}], [(t + 2).strftime("%Y-%m-%d %H:%M:%S %z"), 3, {}], [(t + 4).strftime("%Y-%m-%d %H:%M:%S %z"), 2, {}]], conn.entries(['coffees', 'total/2s'])[:data]
